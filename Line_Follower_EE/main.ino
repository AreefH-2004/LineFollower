...
void weighted_logic(int &irLeftVal,  int &leftState, int &centerState, int &rightState, int &irRightVal) {
    ...
    else if (movement > -1 && movement <= -0.06) {  // Corrected boundary check
        // Slight left veer
        // servoleft.writeMicroseconds(1300);  
        // servoright.writeMicroseconds(1700);
        Serial.println("Robot: S Left");
    } 
    else if (movement > 0 && movement <= 0.6) {
        // Slight right veer
        // servoleft.writeMicroseconds(1700);
        // servoright.writeMicroseconds(1300);
        Serial.println("Robot: S Right");
    }
    else if (movement <= 0 && movement ) {
        // Hard left turn
...
void loop() {
  ...
  int Right_ir = sensor_value(irRightVal_temp);
    ...
    Serial.print(" | irLeftVal: "); Serial.print(rightState);  // Moves to new line
